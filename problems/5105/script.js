(() => {
  ...
  const PROBLEM=Object.freeze({platform:"SWEA",number:"5105",level:"D3",title:"미로의 거리",autoplayMs:560,defaultSampleId:"one",
    sourceSteps:[
      ...
      {text:"queue.append((i, j, 0))",types:"enqueueStart",occurrence:1},
      {text:"found = False",types:"initFound",occurrence:1},
      {text:"found = True",types:"found",occurrence:1},
      ...
      {text:"break",types:"breakGoal",occurrence:3},
      {text:"while queue and not found:",types:"while",occurrence:1},
      {text:"r, c, distance = queue.pop(0)",types:"pop",occurrence:1},
  ...
  function buildSteps(){
    ...
    state.cases.forEach((data,index)=>{
      const {N}=data,original=data.maze,maze=original.map(row=>[...row]);
      // Append-only queue entries and discovery times are shared by all frames.
      // A frame retains only its head/tail and step number, so seeking stays exact.
      const entries=[],discovered=new Map();
      let head=0,current=null,next=null,distance=null,ans=0,check="준비",found=false,foundGoal=null;
      const key=(r,c)=>r*N+c;
      const push=(phase,message)=>steps.push({N,tc:index+1,original,entries,discovered,
        head,tail:entries.length,current,next,distance,ans,check,found,foundGoal,
        phase,line:stepLineMap.get(phase),message,output,sequence:steps.length});
      const enqueue=(r,c,d)=>{
        discovered.set(key(r,c),{distance:d,sequence:steps.length,entry:entries.length});
        entries.push([r,c,d]);
      };
      ...
      for(let i=0;i<N;i++){
        for(let j=0;j<N;j++){
          if(maze[i][j]===2){
            check="출발점 발견";push("findStart",`출발점 2의 위치 (${i}, ${j})를 찾았습니다.`);
            enqueue(i,j,0);check="출발점 enqueue";
            push("enqueueStart","출발점을 (행, 열, 거리 0) 튜플로 큐에 넣습니다.");
            push("breakStart","안쪽 for문을 종료합니다.");break;
          }
        }
        if(entries.length){
          push("hasStart","큐에 출발점이 있으므로 if queue 조건이 참입니다.");
          push("breakSearch","바깥쪽 for문도 종료하고 시작점 탐색을 마칩니다.");break;
        }
      }
      push("initFound","ans = 0, found = False로 BFS를 준비합니다.");
      const direction=[[-1,0],[1,0],[0,-1],[0,1]];
      while(head<entries.length&&!found){
        check="queue 있음 · found = False";
        push("while",`큐에 ${entries.length-head}개 위치가 있고 아직 도착하지 않아 BFS를 계속합니다.`);
        current=entries[head++];distance=current[2];next=null;check="pop(0)";
        push("pop",`큐의 맨 앞 튜플을 꺼내 r = ${current[0]}, c = ${current[1]}, distance = ${distance}로 나눕니다.`);
        for(const [dr,dc] of direction){
          check=`델타 (${dr}, ${dc})`;push("direction",`델타 (${dr}, ${dc}) 방향을 확인합니다.`);
          const nr=current[0]+dr,nc=current[1]+dc;next=[nr,nc];
          push("next",`다음 후보 좌표는 (${nr}, ${nc})입니다.`);
          const inside=0<=nr&&nr<N&&0<=nc&&nc<N;
          check=inside?"범위 안":"범위 밖";
          push("bounds",inside?`(${nr}, ${nc})는 미로 범위 안입니다.`:`(${nr}, ${nc})는 미로 밖이므로 건너뜁니다.`);
          if(!inside)continue;
          if(maze[nr][nc]===0){
            check="이동 가능한 0";push("path",`(${nr}, ${nc})는 통로 0입니다.`);
            enqueue(nr,nc,distance+1);check=`enqueue · 거리 ${distance+1}`;
            push("enqueue",`(${nr}, ${nc}, ${distance+1})을 큐의 뒤에 넣습니다.`);
            maze[nr][nc]=1;discovered.get(key(nr,nc)).visitedAt=steps.length;
            check="방문 처리";push("visit","방금 큐에 넣은 칸을 1로 바꿔 중복으로 큐에 들어가는 것을 막습니다.");
          }else if(maze[nr][nc]===3){
            foundGoal=[nr,nc];check="도착점 3 발견";
            push("goal",`(${nr}, ${nc})에서 도착점 3을 처음 발견했으므로 최단 거리입니다.`);
            ans=distance;check=`ans = ${ans}`;
            push("answer",`도착 칸은 제외하고 지나온 통로 ${distance}칸을 ans에 저장합니다.`);
            found=true;check="found = True";
            push("found","도착 여부를 True로 바꿉니다. ans가 0이어도 도착했음을 구분합니다.");
            push("breakGoal","break로 방향 탐색 for문을 종료합니다.");break;
          }else{
            check=maze[nr][nc]===2?"출발점":"벽 또는 방문한 칸";
            push("path",`(${nr}, ${nc})는 통로 0이 아니므로 큐에 넣지 않습니다.`);
          }
        }
      }
      check=found?"found = True · 종료":"queue 비어 있음 · 종료";
      push("while",found?"not found가 거짓이므로 남은 큐와 관계없이 BFS를 종료합니다.":"큐가 비어 BFS를 종료합니다. 도착하지 못했으므로 ans는 0입니다.");
      output+=`#${index+1} ${ans}\n`;
  ...
  function renderMaze(container,step){
    ...
    if(!view||view.original!==step.original){
      ...
      const fragment=document.createDocumentFragment(),cells=[];
      for(let r=0;r<step.N;r++)for(let c=0;c<step.N;c++){
        const cell=document.createElement("div"),label=document.createTextNode(""),small=document.createElement("span");
        small.className="cell-distance";cell.append(label,small);fragment.appendChild(cell);cells.push({cell,label,small});
      }
      container.appendChild(fragment);view={original:step.original,cells};mazeViews.set(container,view);
    }
    for(let r=0;r<step.N;r++)for(let c=0;c<step.N;c++){
      const {cell,label,small}=view.cells[r*step.N+c],v=step.original[r][c];
      const visit=step.discovered.get(r*step.N+c),visible=visit&&visit.sequence<=step.sequence;
      const classes=["maze-cell"];
      if(v===1)classes.push("wall");if(v===2)classes.push("start");if(v===3)classes.push("goal");
      if(visible&&v===0&&visit.visitedAt<=step.sequence)classes.push("visited");
      if(visible&&visit.entry>=step.head&&visit.entry<step.tail)classes.push("queued");
      ...
      cell.className=classes.join(" ");label.textContent=v===0?"":String(v);
      small.hidden=!visible;small.textContent=visible?visit.distance:"";
    }
  }
  function renderQueue(container,step){container.replaceChildren();if(step.head===step.tail){const p=document.createElement("p");p.className="queue-empty";p.textContent="큐가 비어 있습니다.";container.appendChild(p);return;}step.entries.slice(step.head,step.tail).forEach((q,i)=>{const item=document.createElement("div");item.className="queue-item"+(i===0?" front":"");item.innerHTML=`(${q[0]}, ${q[1]})<small>distance ${q[2]}</small>`;container.appendChild(item);});}
  ...
  function render({follow=true}={}){
    ...
    els.codeLineLabel.textContent=`LINE ${step.line}`;els.boardLabel.textContent=`#${step.tc} · N=${step.N}`;els.phaseLabel.textContent=step.phase.toUpperCase();els.currentValue.textContent=cur;els.distanceValue.textContent=step.distance??"—";els.nextValue.textContent=next;els.queueLengthValue.textContent=(step.tail-step.head);els.checkValue.textContent=`${step.found?"True":"False"} · ${step.check}`;els.answerValue.textContent=step.ans;els.explainText.textContent=step.message;els.queueLabel.textContent=(step.tail-step.head)?`FRONT → ${(step.tail-step.head)}개 대기`:"QUEUE EMPTY";els.outputView.textContent=step.output||"아직 출력이 없습니다.";
    els.mobileCoord.textContent=`#${step.tc} · N=${step.N}`;els.mobileCodeStatus.textContent=`LINE ${step.line}`;els.mobilePhase.textContent=step.phase.toUpperCase();els.mobileExplanation.textContent=step.message;els.mobileStateMeta.textContent=`found = ${step.found?"True":"False"} · ${step.check}`;els.mobileCurrent.textContent=cur;els.mobileDistance.textContent=step.distance??"—";els.mobileNext.textContent=next;els.mobileQueueLength.textContent=(step.tail-step.head);els.mobileAnswer.textContent=step.ans;els.mobileOutputView.textContent=step.output||"아직 출력이 없습니다.";
